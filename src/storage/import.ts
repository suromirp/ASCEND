...
import { ProgramsRepo, SessionTemplatesRepo, PlannedSessionsRepo, SessionLogsRepo, ObjectivesRepo, MilestoneProgressRepo, InjuryNotesRepo, MetaRepo, SettingsRepo, wipeAllData, type AppSettings } from './database';
...
export async function importFromFile(file: File): Promise<void> {
  ...
  // Absent on any export made before this field existed — an empty array is
  // the correct fallback for the write itself, not a claim the backup had
  // no injuries.
  for (const n of (migrated.injuryNotes as InjuryNote[]) ?? []) await InjuryNotesRepo.put(n);
